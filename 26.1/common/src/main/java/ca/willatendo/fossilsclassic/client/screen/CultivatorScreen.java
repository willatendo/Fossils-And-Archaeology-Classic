...
public class CultivatorScreen extends AbstractRecipeBookScreen<CultivatorMenu> {
    ...
    public void extractBackground(GuiGraphicsExtractor guiGraphicsExtractor, int xMouse, int yMouse, float a) {
        ...
        if (this.menu.isOn()) {
            ...
            guiGraphicsExtractor.blitSprite(RenderPipelines.GUI_TEXTURED, BIOMASS_PROGRESS_SPRITE, 14, 14, 0, 14 - onProgess, leftPos + 81, topPos + 36 + 14 - onProgess, 14, onProgess);
        }

        int cultivationProgess = this.menu.getCultivationProgress();
        guiGraphicsExtractor.blitSprite(RenderPipelines.GUI_TEXTURED, CULTIVATION_PROGRESS_SPRITE, 22, 9, 0, 0, leftPos + 77, topPos + 23, cultivationProgess + 1, 9);
    }
}
