...
public class AxolotlspawnBlock extends Block {
    ...
    public void onPlace(BlockState blockState, Level level, BlockPos blockPos, BlockState blockState2, boolean bl) {
        level.scheduleTick(blockPos, this, AxolotlspawnBlock.getAxolotlspawnHatchDelay(level.getRandom()));
    }

    private static int getAxolotlspawnHatchDelay(RandomSource randomSource) {
    ...
    public void tick(BlockState blockState, ServerLevel serverLevel, BlockPos blockPos, RandomSource randomSource) {
        ...
        this.hatchAxolotlspawn(serverLevel, blockPos, randomSource);
    }

    @Override
    protected void entityInside(BlockState blockState, Level level, BlockPos blockPos, Entity entity, InsideBlockEffectApplier insideBlockEffectApplier, boolean intersects) {
    ...
    private static boolean mayPlaceOn(BlockGetter blockGetter, BlockPos blockPos) {
        ...
        FluidState fluidStateAbove = blockGetter.getFluidState(blockPos.above());
        return fluidState.getType() == Fluids.WATER && fluidStateAbove.getType() == Fluids.EMPTY;
    }

    private void hatchAxolotlspawn(ServerLevel serverLevel, BlockPos blockPos, RandomSource randomSource) {
        this.destroyBlock(serverLevel, blockPos);
    ...
    private void spawnTadpoles(ServerLevel serverLevel, BlockPos blockPos, RandomSource randomSource) {
        ...
        for (int j = 1; j <= i; ++j) {
            ...
            axolotl.snapTo(x, (double) blockPos.getY() - 0.5, z, yRot, 0.0f);
