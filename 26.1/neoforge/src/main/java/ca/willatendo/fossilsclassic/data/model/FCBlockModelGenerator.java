...
public final class FCBlockModelGenerator extends SimpleBlockModelGenerator {
    ...
    private void createAnalyzer(Block block) {
        ...
        Identifier capTexture = TextureMapping.getBlockTexture(block, "_cap");
        Identifier frontTexture = TextureMapping.getBlockTexture(block, "_front");
        Identifier frontTextureActive = TextureMapping.getBlockTexture(block, "_front_active");
    ...
    private void createCultivator(Block block) {
        Identifier sideTexture = TextureMapping.getBlockTexture(block, "_side");
        Identifier sideTextureActive = TextureMapping.getBlockTexture(block, "_side_active");
    ...
    private void createArchaeologyWorkbench(Block block) {
        Identifier booksTexture = TextureMapping.getBlockTexture(block, "_books");
        Identifier sideTexture = TextureMapping.getBlockTexture(block, "_side");
        Identifier topTexture = TextureMapping.getBlockTexture(block, "_top");
        Identifier topTextureActive = TextureMapping.getBlockTexture(block, "_top_active");
        Identifier bottomTexture = TextureMapping.getBlockTexture(Blocks.SPRUCE_PLANKS);
    ...
    private void createJurassicFern(Block block) {
        ...
        ExtendedModelTemplate extendedModelTemplate = ModelTemplates.CROP.extend().renderType("cutout").build();
        for (DoubleBlockHalf doubleBlockHalf : DoubleBlockHalf.values()) {
            for (int growthStage = 0; growthStage < 6; growthStage++) {
                ...
                MultiVariant jurassicFern = BlockModelGenerators.plainVariant(extendedModelTemplate.createWithSuffix(block, "_" + doubleBlockHalf.getSerializedName() + "_" + growthStage, new TextureMapping().put(TextureSlot.CROP, this.modLocation("block/jurassic_fern_" + doubleBlockHalf.getSerializedName() + "_" + labledGrowthStage)), this.modelOutput));
    ...
    private void createDrum(Block block, String... commands) {
        ...
        for (String command : commands) {
            Identifier topTexture = TextureMapping.getBlockTexture(block, "_top_" + command);
    ...
    private void createFeeder(Block block) {
        Identifier sideTexture = TextureMapping.getBlockTexture(block, "_side");
        Identifier frontTexture = TextureMapping.getBlockTexture(block, "_front");
        Identifier topTextureEmpty = TextureMapping.getBlockTexture(block, "_top_empty");
        Identifier topTextureFull = TextureMapping.getBlockTexture(block, "_top_full");
        Identifier bottomTexture = TextureMapping.getBlockTexture(Blocks.IRON_BLOCK);
    ...
    private void createAxolotlspawn(Block block) {
        ...
        MultiVariant axolotlspawn = BlockModelGenerators.plainVariant(FCModelTemplates.FROGSPAWN.create(block, new TextureMapping().put(TextureSlot.TEXTURE, this.modLocation("block/axolotlspawn")), this.modelOutput));
